...
const initRepo = () => {
    ...
    writeFileSync(join(dir, 'src', 'app.js'), ["console.log('start');", "console.log('middle');", "console.log('end');"].join('\n'), 'utf8');
    git(['add', '.']);
    git(['commit', '-m', 'init']);
    return { dir, git };
};
...
const createFetchMock = (requests, body, status = 200) => async (url, options) => {
    requests.push({ url, options: { ...(options ?? {}) } });
    return new Response(JSON.stringify(body), { status });
};
const isGraphqlBody = (value) => {
    ...
    const expectedHeadOid = input.expectedHeadOid;
...
test('createGithubCommit builds request with trailers and summary', async (t) => {
    await withRepo(t, async (dir, git) => {
        ...
        const bodyText = typeof firstRequest.options.body === 'string' ? firstRequest.options.body : '{}';
